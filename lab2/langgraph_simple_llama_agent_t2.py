...
def create_graph(llm,llm2=None):
    ...
    # =========================================================================
    # NODE 2: call_both_llms (PARALLEL EXECUTION)
    # =========================================================================
    # This node takes the user input from state and calls BOTH LLMs simultaneously
    # using ThreadPoolExecutor for true parallel execution.
    # State changes:
    #   - user_input: Unchanged (read only)
    #   - llm_response: Set to Llama's response
    #   - llm_response2: Set to Qwen's response
    def call_both_llms(state: AgentState) -> dict:
        """
        Node that invokes BOTH LLMs simultaneously with the user's input.

        Reads state:
            - user_input: The text to send to both LLMs
        Updates state:
            - llm_response: The text generated by Llama
            - llm_response2: The text generated by Qwen
        """
        ...
        global verbose_flag
        if verbose_flag:
            print("\nProcessing with both LLMs simultaneously...")

        # Define functions to call each LLM
        def call_llama():
        ...
        # Execute both LLM calls in parallel using ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=2) as executor:
            future_llama = executor.submit(call_llama)
            future_qwen = executor.submit(call_qwen)
            
            # Wait for both to complete and get results
            response1 = future_llama.result()
            response2 = future_qwen.result()

        # Return both responses
        return {"llm_response": response1, "llm_response2": response2}    
    ...
    def route_after_input(state: AgentState) -> str:
        ...
        # Default: Proceed to both LLMs (even for empty input)
        return "call_both_llms"
    ...
    graph_builder.add_node("call_both_llms", call_both_llms)  # Single node that runs both LLMs in parallel
    ...
    # 2. get_user_input -> [conditional] -> call_both_llms OR END
    ...
    graph_builder.add_conditional_edges(
        ...
        {
            "call_both_llms": "call_both_llms",  # Any input -> proceed to both LLMs (parallel)
            "get_user_input": "get_user_input",  # Empty input -> loop back
            END: END                              # Quit command -> terminate graph
        }
    )

    # 3. call_both_llms -> print_response (both LLMs run in parallel within the node)
    graph_builder.add_edge("call_both_llms", "print_response")
